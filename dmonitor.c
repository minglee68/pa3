...
int check_lock(int orig_thread, int mutex_flag, char * output);
int check_try(int orig_thread, int thread_flag, char * output);
...
int check_lock(int orig_thread, int mutex_flag, char * output) {
	...
	for(i = 0; i < 10; i++) {
		if (lock[i][mutex_flag] == 1) {
			...
			if (i == orig_thread)
				return -1;
			return check_try(orig_thread, i, output);
		}
	}
...
int check_try(int orig_thread, int thread_flag, char * output) {
	...
	for (i = 0; i < 100; i++) {
		if (try[i][thread_flag] == 1) {
			...
			return check_lock(orig_thread, i, output);
		}
	}
...
int pthread_mutex_lock(pthread_mutex_t *mutex) {
	...
	if (check_lock(thread_flag, mutex_flag, output) == -1)
